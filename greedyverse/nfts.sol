...
contract greedyverseNfts is ERC1155, Ownable{
    ...
    function _mint(uint256 id, uint256 amount) internal {
         ...
         require(mintedNftsAmount[id] + amount < maxNftsAmount[id], "Mint amount not available");
         require(msg.value >= nftMintPrice[id] * amount, "Amount too small to mint nft");
         if(id == 2){
             require((balanceOf(msg.sender,id) + amount <= MaxWoodWall_per_player), "You can not mint more than 40 wood walls");
         }else if(id == 3){
         ...
         depositProceeds();
         mintedNftsAmount[id] = mintedNftsAmount[id] + amount;
    ...
    function depositProceeds() private {
        ...
        sendingLocked = true;
        (bool success1, ) = gameContract.call{value: msg.value / 2}("");
        require(success1, "Failed to deposite to gameContract");
        (bool success2, ) = teamWallet.call{value: msg.value / 2}("");
    ...
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public override {
        require(
         ...
         if(id == 2){
             require((balanceOf(to,id) + amount <= MaxWoodWall_per_player), "No address can own more than 40 wood walls");
         }else if(id == 3){
